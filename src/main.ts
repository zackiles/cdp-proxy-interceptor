    ...
    try {
      ...
      for await (const { isFile, name } of entries) {
        if (
          ...
          name.toLowerCase().includes('.disabled.') &&
            console.log(`[PLUGINS] Skipping disabled plugin: ${name}`)
          continue
        }
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(
          new Error('Chrome WebSocket connection timeout after 30 seconds'),
        )
    ...
    console.log(
      `[CDP PROXY] Successfully established WebSocket connection for ${path}`,
    )
    return response
  } catch (error) {
