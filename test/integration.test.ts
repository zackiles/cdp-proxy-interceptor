...
async function retryConnect(port: number, maxRetries = 5, retryDelay = 2000) {
  ...
  for (let i = 0; i < maxRetries; i++) {
    try {
      console.log(`[TEST] Attempting to connect to proxy (attempt ${i + 1}/${maxRetries})...`)
  ...
  await t.step('should successfully connect to proxy and navigate', async () => {
    try {
      ...
      try {
        ...
        if (!response.ok) {
          console.error('[TEST] WebSocket URL not available:', response.status, response.statusText)
        } else {
